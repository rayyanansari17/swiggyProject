...
//     let check = await restaurantModel.updateOne(
//       { "verifyToken.emailToken": eToken },
//       { $set: { "isVerified.email": true, "verifyToken.emailToken": null } },
//       { new: true }
//     );
//     res.status(200).json({ check,msg: "Email address is now verified" });
//   } catch (error) {
//     console.log(error);
//     res.status(500).json({ msg: error });
//   }
...
// router.get("/verify-email/:emailToken", async (req, res) => {
//   try {
//     let pToken = req.params.emailToken;
//     let user = await restaurantModel.findOne({
//       "verifyToken.emailToken": pToken,
//     });
...
//       { new: true }
//     );
//     res.status(200).json({ msg: "Your email is now verified" });
//   } catch (error) {
//     console.log(error);
//     res.status(500).json({ msg: error });
//   }
...
router.get("/verify-email/:emailToken", async (req, res) => {
  try {
    const pToken = req.params.emailToken;
    ...
    if (!user) {
      return res.status(400).json({ msg: "Invalid user or link" });
    }

    return res.status(200).json({
      msg: "Email address is now verified"
    });
